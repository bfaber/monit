...
LogReader::LogReader(std::vector<ConfigItem*> *cfgs, MatchProcessor *csvprocessor) :
    threadStarted(false),
    threadStopped(false),
    shutdown(false),
    configs(cfgs) {
 ...
 * readFile will use the interface provided by the CSVProcessor for dealing with events.  The events vector will be allocated here and then moved to the CSVProcessor via uniquepointer.*  TODO:  configItems should be bundled by filename so that all regexes assoc with a file
