...
LogReader::LogReader(std::vector<ConfigItem*> *cfgs, MatchProcessor *csvprocessor) :
	...
	const char* error;
	int error_offset;
	const pcre* compiled_regex = pcre_compile(configitem->getRegex().c_str(), 0, &error, &error_offset, NULL);
	if(error_offset != 0) {
...
bool LogReader::readFile() {
	...
	if( logfile.is_open() ) {
		...
		int rc = pcre_exec(compiledRegex, NULL, line.c_str(), strlen(line.c_str()), 0, 0, captures, 30);
